...
if (strlen($_SESSION['login']) == 0) {
  ...
  if (isset($_POST['addcart'])) {
    ...
    if (empty($productcode) || empty($quantity) || empty($total) || empty($productimage) || empty($productname) || empty($productprice)) {
      // Handle the validation error, redirect or show an error message
      ...
      // Insert into history table (assuming you have the same validation logic for the history table as well)
